...
public class Sensor_Image implements PlugIn{
	
	private String fileLocation;
	private String[] operators = {"Add","Subtract","Multiply","Divide", "AND", "OR", "XOR", "Min", "Max", "Average", "Difference", "Copy", "Transparent-zero"};
    private String[] lcOperators = {"add","sub","mul","div", "and", "or", "xor", "min", "max", "ave", "diff", "copy", "zero"};
    ...
    public Sensor_Image(){
    	new ImageJ();
    	this.fileLocation = "/home/pi/Pictures";
    }
    
    public Sensor_Image( String fileLoc ){
    	new ImageJ();
    ...
    public void setSizeThreshold(int val) {
    	this.sizeThreshold = val;
    }
    
    public int countClusters(String file1, String file2) {
    	
		ImagePlus imp1 = IJ.openImage( this.fileLocation + "/" + file1);
		ImagePlus imp2 = IJ.openImage( this.fileLocation + "/" + file2);
		
		imp1.show();
		this.run("sub", imp1, imp2);

		IJ.selectWindow(file1);
		IJ.run("16-bit");
		IJ.setAutoThreshold(imp1, "Huang");
		Prefs.blackBackground = false;
		IJ.run(imp1,"Convert to Mask","");
		IJ.run(imp1,"Convert to Mask","");
		IJ.run(imp1,"Convert to Mask","");
		IJ.run(imp1,"Convert to Mask","");
		IJ.makeRectangle(110,0,90,180);
		IJ.run("Crop");
		IJ.run("Analyze Particles...", "  show=[Overlay Masks] display include");
		
		int count=0;
		try {
			ResultsTable rt = ResultsTable.getResultsTable();
			for(float f : rt.getColumn( rt.getColumnIndex("Area") ) ) {
				if( f > this.sizeThreshold ) {
					count++;
				}
			}
		}catch(Exception e) {
